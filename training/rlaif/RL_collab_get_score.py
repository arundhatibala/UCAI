...
model_reward = AutoModelForCausalLM.from_pretrained("testreward/", local_files_only=True)
...
for epoch, batch in tqdm(enumerate(ppo_trainer.dataset)):
    ...
    reward = get_score(model_reward, tokenizer, batch["query"], batch["response"])
    print("this is the last REWARD: ", reward, " and its type: ", type(reward))
